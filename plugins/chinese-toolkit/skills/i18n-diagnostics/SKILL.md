...
npm test
```
必须确保以下 3 大测试套件 100% 全部 PASS：
1. `test/verify.js`（34+ 核心 DOM 渲染与 Monaco Editor 保护断言）
2. `test/test-screenshots.js`（54+ 真实界面截图用例）
3. `test/test-menu-and-titles.js`（窗口菜单与用户自定义会话标题防误伤断言）
